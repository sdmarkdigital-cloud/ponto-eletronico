...
import { CalculatorIcon, CameraIcon, ClockIcon, CogIcon, DocumentReportIcon, DownloadIcon, KeyIcon, LocationMarkerIcon, PdfIcon, PencilIcon, TrashIcon, UploadIcon, UserGroupIcon } from '../../../components/icons';
import TimePicker from '../../../components/TimePicker';
import * as api from '../../../services/api';
import { supabase } from '../../../services/supabase';
import { AdminJustificationType, Benefits, ClockType, DailyBalance, Justification, Payslip, Role, Sector, ServiceReport, TimeBankReport, TimeClockEntry, User, WorkHours } from '../../../typings';
...
// FIX: Completed the component, added authentication checks, and adapted it to the Next.js layout structure.
const AdminDashboard: React.FC = () => {
  const { user, loadingSession } = useContext(AuthContext);
  const router = useRouter();

  useEffect(() => {
    if (!loadingSession && (!user || user.role !== Role.ADMIN)) {
      router.replace('/auth');
    }
  ...
  const handleDownloadReport = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const user of usersToReport) {
        if (usersToReport.indexOf(user) > 0) {
          doc.addPage();
  ...
  const excluirAcesso = async (funcionario: User) => {
    setLoadingButtonId(funcionario.id);
    try {
      console.log('🗑️ Excluindo acesso de:', funcionario.name);
      ...
      console.log('✅ Acesso excluído no banco');
  ...
  const handleExcluirAcessoClick = (funcionario: User) => {
    setConfirmationModal({
      isOpen: true,
      title: 'Excluir Acesso',
      message: `Excluir acesso de ${funcionario.name}?`,
      onConfirm: () => excluirAcesso(funcionario),
    });
  };
  ...
  if (loadingSession || !user) {
    return (
      <div className="flex items-center justify-center h-full bg-primary text-text-muted">
        Verificando acesso...
      </div>
    );
  }
