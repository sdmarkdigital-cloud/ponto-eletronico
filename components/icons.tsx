...
// FIX: To resolve a TypeScript error where the `title` prop was not recognized on SVG components,
// a custom `IconProps` type is defined to explicitly include `title`. The components are
// updated to destructure the `title` prop and render it as an accessible `<title>`
// element inside the SVG, which is better for accessibility.
...
export const DocumentReportIcon: React.FC<IconProps> = ({ title, ...props }) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" {...props}>
    {title && <title>{title}</title>}
...
export const RouteIcon: React.FC<IconProps> = ({ title, ...props }) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" {...props}>
    {title && <title>{title}</title>}
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 5h8m0 0v8m0-8l-8 8-4-4-6 6" />
  </svg>
);
